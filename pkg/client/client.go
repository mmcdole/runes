...
func New(scriptPath string) (*Client, error) {
	...
	// Create viewport
	client.viewport = components.NewViewport(term, client.buffer)
	if err := client.layout.RegisterComponent(layout.ViewportType, client.viewport); err != nil {
		return nil, fmt.Errorf("failed to register viewport: %w", err)
	}

	// Create input
	client.input = components.NewInputBar(term, nil)
	if err := client.layout.RegisterComponent(layout.InputBarType, client.input); err != nil {
		return nil, fmt.Errorf("failed to register input: %w", err)
	}

	// Create status
	client.status = components.NewStatusBar(term)
